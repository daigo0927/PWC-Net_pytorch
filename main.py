...
def train(args):
    ...
    for step in range(1, args.total_step + 1):
        ...
        if step % args.summary_interval == 0:
            ...
            B = flows[0].size(0)
            for b in range(B):
                batch = [vis_flow(i) for l in range(len(flows)) for i in np.split(np.array(F.upsample(flows[l][b].unsqueeze(0), scale_factor = 2 ** (6-l)).detach().squeeze(0)).transpose(1,2,0))]
