...
def main():
    ...
    # loss
    train_parser.add_argument('--weights', nargs = '+', type = float, default = [1,1,0.32,0.08,0.02,0.01,0.005])
    ...
    for step in range(1, args.total_step + 1):
        ...
        if step % args.summary_interval == 0:
            ...
            for b in range(B):
                ...
                vis = np.concatenate(batch + [vis_flow(np.array(flows[-1][b].detach()).transpose(1,2,0)), vis_flow(flow_gt[b].detach().cpu().numpy().transpose(1,2,0))], axis = 1)
